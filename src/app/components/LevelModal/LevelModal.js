...
const LevelModal = ({ data, handleCloseModal }) => {
  ...
  return (
    <div
      className={`flex flex-col md:flex-row lg:flex-row items-center justify-center w-screen h-screen ${myFont.className} relative bg-black bg-opacity-70`}
    >
      ...
      <button
        onClick={() => {
          router.push("/");
          toast.success("Level Completed", "success");
        }}
        className="text-4xl z-10 text-white absolute top-[2rem] left-[2rem] md:top-[5rem] lg:top-[5rem] md:left-[6rem] lg:left-[6rem] hover:cursor-pointer"
      >
        X
      </button>
      <div className="modal">
        <div className="modal-content">
          ...
          {DataChanged ? (
            <div>{data.vilMainContent}</div>
          ) : (
            <div>{data.heroMainContent}</div>
          )}
        </div>
      </div>
      {/* </div> */}
